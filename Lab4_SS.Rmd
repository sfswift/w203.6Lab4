...
```{r, include=FALSE}
#Take the median foe each county from all wage variables.
...
#review the more aggreate location data
table(crime$west)
table(crime$central)
table(crime$urban)
...
Running a summary on model1 using the Robust standard errors to account for the heteroskedacity we get:
...
Next, we wanted to refine our model with more independent variables, and we chose to add `pctymle` and `pctmin80` incorporate some demographic information in our model. Since the `pctymle` distribution was skewed from the analysis of the histogram, we decided to use the log form of the variable in the analysis. The percent of young males (`pctymle`) and the percent minority in 1980 (`pctmin80`) are both reasonable variables to include in this model, considering males tend to be arrested and incarcerated more often than females and (unfornately) underrepresented groups tend to be associated with higher crime, so the relationship is worth exploring. Though the `pctmin80` data is 7 years old, we chose to include it in our model so we could somewhat assess racial diversity.
...
Zero conditional Mean:  Let us examine the zero conditional mean assumption by looking at the plot of residuals vs fitted values 
...
From the correlation values of the residuals to the independent variables, it is clear that exogenity assumption holds. (The way the variables have been defined, also rules out any endogenous relationship.)
...
Further supporting this violation, the Breusch-Pagan test has a significant p-value. We will be conservative and use the robust standard errors we have produced above to address any heteroskedacity in the data.
...
From the above graphs, it is clearly that the errors are mostly normal, except in the extremes where we see a few outliers. But it is pretty clear that the assumption is true. 

Finally examining the summary of model2 and using the standard errors : 
...
Now, let us examine a model, that includes the previous covariates and along with the probability of arrests (`prbarr`) and the median wage (`med_wag`) to assess the robustness of our original models. Also from the initial analysis of `prbarr`, we concluded that the sample distribution is skewed and it would better to use the log transformed variable. 
...
Homoskedasticity:
To assess homoskedasticity, let us look at the graph of standardized residuals vs the fitted values below
...
From both these graphs, it is clear that the residuals are normal. 

Finally taking a summary of model3 using the robust standard errors :
```{r}
s3 <- summary(model3) 
...
Although our results are informative, no causal relationship can be drawn from our models. In general, regression itself does not imply causality; it's basis is in correlation. In this particular instance, this general rule holds. The data we have is observational, and no experiment was performed.
