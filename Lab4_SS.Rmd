...
Next, we wanted to refine our model with more independent variables, and we chose to add the `pctymle` variable and the `pctmin80` variable to our model. Since the `pctymle` distribution was skewed from the analysis of the histogram, we decided to use the log form of the variable in the analysis. The percent of young males and the percent minority in 1980 are both reasonable variables to include in this model, considering males tend to be arrested and incarcerated more often than females and (unfornately) underrepresented groups tend to be associated with higher crime. Though the `pctmin80` data is 7 years old, we chose to include it in our model so we could somewhat assess racial diversity.
...
3>  No perfect Collinearity : It is clear from the correlation value of the indpendent variables taken pairwise : `cor(log_pctymle), log_density)`, `cor(log_pctymle), pctmin80)` and `cor(pctmin80, log_density)` that none of the indpendent variables are collinear. 
